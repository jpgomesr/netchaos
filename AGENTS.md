...
`netchaos` ([README](README.md)) is a Go library providing simulated `net.Conn`/`net.Listener` with deterministic fault injection. **Current stage: M1-M9 implemented and godoc'd; tagged and released as `v0.3.0`** (2026-09-16), on top of `v0.2.0`'s six-fault-kind surface, plus the `M8` bug fixes and tooling additions that landed on `main` between `v0.2.0` and this tag (`M8-1`..`M8-6`). The module (`github.com/jpgomesr/netchaos`, `go 1.25`) has a working `Network` type (`NewNetwork`, `Dial`/`DialContext`/`DialerFor` (now with `DialerOption`/`WithDialTimeout` for a bounded wait against a partitioned peer), `Listen`, `WithSeed`) with the full shipped fault set implemented and composed in one fixed order: `WithLatency`, `WithPacketLoss`, `WithBandwidth`, `WithDuplication`, `WithCorruption`, and `WithPartition`/`Network.Partition`/`Network.Heal` (plus `WithPeerName`, added during v0.1.0 to make a dialer partition-targetable; `Partition`/`Heal`/`Reset` now panic on an empty peer name or a self-pair, matching `WithPartition`'s own check), alongside `Network.Reset` (imperative mid-stream reset), the live setters `Network.SetLatency`/`Network.SetPacketLoss`/`Network.SetDuplication`/`Network.SetCorruption` (only `WithBandwidth` remains construction-time-only), the exported `Network.Trace` (each `FaultEvent` also carries `Size`/`CorruptedByte`/`CorruptedBit`), and the tuning options `WithPipeBound`/`WithListenerBacklog`. Sentinel errors live in `errors.go` (`ErrUnsupportedNetwork`, `ErrConnectionRefused`, `ErrAddressInUse`, `ErrBacklogFull`). The API is stable but not frozen until `v1.0.0` — the M5-2 ergonomics review the v0.2.0 surface never got (`docs/04-api-design.md`, issue #75) ran as `M8-7` and produced the decisions [M9 — v1.0.0 surface additions](docs/tasks/m9-v1-surface-additions.md) implemented (`#83`, `#86`, `#78` in part, `#85` in part), now closed; tagging `v1.0.0` remains the maintainer's own usage-evidence-driven call. See [docs/07-contributing.md](docs/07-contributing.md). Check `CHANGELOG.md` and the repo's tags for the current release state; do not assume unimplemented work exists.
...
`README.md` and `.claude/skills/netchaos/` are the exception: both describe the **last tagged release** by design (the skill's own `go get …@v0.3.0` instruction, README's "Status: `v0.3.0` released" banner). They are updated together, once, in a milestone-close PR — `#71` for `v0.2.0`, [M9-5](docs/tasks/m9-v1-surface-additions.md#m9-5--close-out-m9-sync-readmemd-and-the-skill) for `v0.3.0` — not per feature PR. Don't update them for a change that hasn't shipped in a tag yet; do flag in the relevant milestone's task file when a close-out pass is due, the way `M9-5` did.
