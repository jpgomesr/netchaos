...
// Added by M7-9 (issue #53, candidate 4), post-v0.1.0. Draws unconditionally
...
- `faultKind` separates latency draws, packet-loss draws, and corruption draws ([M7-9](tasks/m7-v0.2.0-implementation.md#m7-9--fault-kind-data-corruption)) from each other on the same connection and direction, so that adding `WithLatency` to an existing test does not shift that test's packet-loss or corruption sequence, and so on for every pair.
...
**Fault composition and draw discipline, permanent as of [M2-5](tasks/m2-determinism-and-faults.md#m2-5--fault-composition-rules):** when more than one fault is configured on the same connection direction, they are evaluated in exactly one fixed order — **partition, then packet loss, then bandwidth, then latency, then corruption** ([M7-9](tasks/m7-v0.2.0-implementation.md#m7-9--fault-kind-data-corruption)) — by a single evaluator, not independent hooks that happen to run in some order. Partition short-circuits before any draw: a partitioned unit is discarded with **zero** stream consumption, since partition must stay deterministic by nature and a draw there would perturb every later unit's loss/latency/corruption sequence on that direction. Packet loss, evaluated next, is the same kind of gate for bandwidth and corruption both: a dropped unit never reaches the link (so it costs no simulated transmission time) and is never corrupted (so there is nothing left for a bit flip to land in).

A unit that clears the partition gate draws from **every configured fault's stream unconditionally** — packet loss's Bernoulli trial, latency's duration, and corruption's Bernoulli trial (and, if corrupting, its byte/bit indices) are all drawn even when an earlier fault in the order already decided to drop the unit (e.g. a unit loss drops still draws a latency duration and a corruption decision it will never use). This keeps each drawing fault's draw index equal to the unit index on that direction, independent of what any other fault decided, which is what makes a fault trace diffable unit-for-unit across runs. Changing this discipline later — including which faults draw unconditionally versus lazily — is a breaking change to this contract, not an implementation detail free to vary.

**This discipline binds fault kinds that draw.** Bandwidth ([M7-5](tasks/m7-v0.2.0-implementation.md#m7-5--fault-kind-bandwidth-throttling)) is the one fault kind that does not: its delay is a deterministic function of a unit's size and the configured rate, with no `faultKind` byte and no derived stream. A kind with no stream has no draw index to misalign, so — the same way partition's zero-draw short-circuit cannot perturb loss or latency — enabling bandwidth cannot perturb any other configured fault's sequence either. Corruption is not an exception the way bandwidth is: it is a per-unit Bernoulli decision with its own stream (`kindCorrupt`), so it draws unconditionally like packet loss and latency. Its byte-index and bit-index draws (`corruptionSite`, `rand.go`) only happen when the Bernoulli trial says yes and the unit is non-empty — a conditional secondary draw, the same shape `uniformDuration`'s rejection-sampling loop already uses (a single logical decision is not necessarily one raw draw), and confined entirely to `kindCorrupt`'s own stream, so it cannot perturb any other fault kind either way.
