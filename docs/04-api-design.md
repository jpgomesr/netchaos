...
- `faultKind` separates latency draws, packet-loss draws, duplication draws ([M7-8](tasks/m7-v0.2.0-implementation.md#m7-8--fault-kind-packet-duplication)), and corruption draws ([M7-9](tasks/m7-v0.2.0-implementation.md#m7-9--fault-kind-data-corruption)) from each other on the same connection and direction, so that adding `WithLatency` to an existing test does not shift that test's packet-loss, duplication, or corruption sequence, and so on for every pair.
...
A unit that clears the partition gate draws from **every configured fault's stream unconditionally** — packet loss's Bernoulli trial, latency's duration, corruption's Bernoulli trial (and, if corrupting, its byte/bit indices), and duplication's Bernoulli trial are all drawn even when an earlier fault in the order already decided to drop the unit (e.g. a unit loss drops still draws a latency duration, a corruption decision, and a duplication decision it will never use). This keeps each drawing fault's draw index equal to the unit index on that direction, independent of what any other fault decided, which is what makes a fault trace diffable unit-for-unit across runs. Changing this discipline later — including which faults draw unconditionally versus lazily — is a breaking change to this contract, not an implementation detail free to vary.

**This discipline binds fault kinds that draw.** Bandwidth ([M7-5](tasks/m7-v0.2.0-implementation.md#m7-5--fault-kind-bandwidth-throttling)) is the one fault kind that does not: its delay is a deterministic function of a unit's size and the configured rate, with no `faultKind` byte and no derived stream. A kind with no stream has no draw index to misalign, so — the same way partition's zero-draw short-circuit cannot perturb loss or latency — enabling bandwidth cannot perturb any other configured fault's sequence either. Corruption and duplication are not exceptions the way bandwidth is: each is a per-unit Bernoulli decision with its own stream (`kindCorrupt`, `kindDuplicate`), so each draws unconditionally like packet loss and latency. Corruption's byte-index and bit-index draws (`corruptionSite`, `rand.go`) only happen when its Bernoulli trial says yes and the unit is non-empty — a conditional secondary draw, the same shape `uniformDuration`'s rejection-sampling loop already uses (a single logical decision is not necessarily one raw draw) — and the duplicate duplication admits reuses the release timing bandwidth/latency already computed for the original rather than drawing its own. Both are confined entirely to their own stream, so neither can perturb any other fault kind either way.
