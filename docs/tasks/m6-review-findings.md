...
**M5-2 confirmed the finding and filed the issue** (its finding F4) rather than deciding it — the change breaks every address string a test prints, so it belongs to [07](../07-contributing.md)'s issue-first process. The decision itself is still open and still this task's.
