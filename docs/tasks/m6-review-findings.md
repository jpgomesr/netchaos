...
**What this milestone deliberately does not do:** it decides nothing on its own. The decision tasks (`M6-10` through `M6-17`) produce a written decision, per the [M0](m0-decisions-and-foundations.md) precedent — they do not authorize the feature they describe.

> **The decisions are now made.** `M6-2`, `M6-10`, `M6-11`, `M6-12`, `M6-13`, `M6-14` and `M6-17` each carry the maintainer's answer on their own `**Decision:**` line, and the accepted scope is recorded in [06 — Scope & Roadmap § Accepted for v0.2.0](../06-scope-and-roadmap.md#accepted-for-v02). **Accepted means in scope, not built.** Only `M6-2` was implemented under this milestone, because its answer was specifically "before `v1.0.0`" and the change was bounded; everything else accepted here needs its own task and its own PR. **Those tasks now exist: [M7 — v0.2.0 implementation](m7-v0.2.0-implementation.md)** is the build these decisions authorize, one task per accepted item, and it is where a reader should go for the implementation status of anything accepted below. `M6-15` and `M6-16` remain `todo` and untouched by design, and `M6-12` was folded into `M6-16`'s gate rather than answered separately. Two of them (`M6-15` reordering, `M6-16` per-peer-pair scoping) restate questions that [06 — Scope & Roadmap](../06-scope-and-roadmap.md) has already gated on "real usage evidence rather than a fixed timeline," and `AGENTS.md` says the reordering question is not to be resolved without the maintainer; neither task reopens either question, they only record that the review reached them and found the gate still closed.
...
> **Superseded in part by [M7-2](m7-v0.2.0-implementation.md#m7-2--networkdialerfor-make-a-drop-in-dialer-partition-targetable).** The paragraph above is right that `Dial` itself can never carry a peer name, and that half stands. What it over-generalized is the word *permanent*: it read the limitation as belonging to anything with `Dial`'s shape, when it belongs to `Dial` specifically. `Network.DialerFor(name)` (issue [#36](https://github.com/jpgomesr/netchaos/issues/36), decided in M7) returns a function with exactly that signature which *does* carry a name, so a drop-in dialer is partition-targetable after all. The godoc cross-reference this task landed stays — an unnamed `n.Dial` is still not targetable, and that is still worth stating from `Partition`'s side — but it now points at two ways to name a dialer rather than describing a dead end.
...
**Status:** done — decided (accepted for `v0.2.0`; implementation is a separate task)
**Decision:** **synthesize a port** (option 1). Addresses gain port structure so `net.SplitHostPort` succeeds against a netchaos address as it does against a real one. Chosen because this is the one finding that bears directly on [01 — Vision](../01-vision.md)'s adoption claim: the cost is currently paid by code the adopter does not want to change, which is the exact friction netchaos exists to remove. It is also the most disruptive item accepted — every address string in test output and godoc examples changes, and the address→peer mapping becomes non-trivial for the first time, so `addr.go`'s single-definition property has to be deliberately re-established rather than assumed. **Must land before `v1.0.0`**, since adding port structure afterwards breaks every address string a test prints.
...
**Status:** done — decided (all four accepted for `v0.2.0`; each implementation is a separate task)
**Decision:** **all four candidates enter `v0.2.0`** — bandwidth throttling, mid-stream connection reset, packet duplication, and data corruption. The question this task said would decide candidates 3 and 4 was answered explicitly rather than by analogy: real TCP does hide duplication and corruption from the application, but it also hides packet *loss* by retransmitting, and [M0-3](m0-decisions-and-foundations.md#m0-3--decide-fault-granularity-per-write-vs-per-simulated-packet) crossed that line deliberately with the silent-gap model, precisely so a test can exercise what the application sees when the transport does not behave. That precedent extends to both.
...
**Status:** done — decided (merged into `M6-16`'s gate; nothing to implement)
**Decision:** **merged into [M6-16](#m6-16--per-peer-pair-scoping-gate-check-only)'s gate rather than decided on its own.** Per-peer-pair and per-direction scoping are two axes of the same configuration model, and shipping one without considering the other is how an option surface becomes incoherent — so the two get designed together when that gate opens, and neither is decided now. This does **not** reopen `M6-16`: the gate stays closed on the same external-usage evidence, of which there is still none.
**Roadmap item:** none
**Depends on:** —
**Blocks:** —
...
**Status:** done — decided (accepted for `v0.2.0`; implementation is a separate task)
**Decision:** **add `SetLatency`/`SetPacketLoss`** (option 1), matching `Partition`/`Heal`'s live semantics. Two consequences are accepted with it and neither is optional: the per-unit read path stops being lock-free, and the [determinism contract](../04-api-design.md#determinism-contract) widens to cover a mid-run configuration change. **The contract must be rewritten before the setters are implemented, not after** — widening the library's core promise is the substantive part of this change rather than a side effect of it, and `docs/04` now carries that instruction at the contract itself.
**Roadmap item:** none
**Depends on:** —
**Blocks:** —
...
**Status:** done — decided (accepted for `v0.2.0`; implementation is a separate task)
**Decision:** **export the full trace** (option 1), not counters only — resolving the deferral `M2-1` recorded. The cost is accepted knowingly rather than overlooked: the `faultEvent` shape becomes public API and therefore a compatibility surface at `v1.0.0`, on a format [M3-3](m3-synctest-and-reproducibility.md) deliberately made high-friction to change. `M2-1`'s deferral note now points here so the two records cannot drift.
...
**Status:** done — decided (accepted for `v0.2.0`; implementation is a separate task)
**Decision:** **both options — `WithPipeBound` and `WithListenerBacklog`** (option 1). Both constants shape observable behaviour and neither is reachable by a user, and the in-package `newConnPairWithBound` constructor is direct evidence the need is real: the bound already had to be varied to test back-pressure. The two extra names are accepted as *input* to [M5-2](m5-hardening-and-ergonomics.md#m5-2--api-ergonomics-review-before-v100)'s surface review rather than as an exemption from it. Note the interaction with [M6-11](#m6-11--decide-which-new-fault-kinds-if-any-enter-v02)'s accepted bandwidth throttling: a throttle slower than the reader is what makes the bound observable in the first place, so the two are worth implementing in that order.
**Roadmap item:** none
**Depends on:** —
**Blocks:** —
