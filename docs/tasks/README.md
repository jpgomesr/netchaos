...
| [M8 — v1.0.0 readiness](m8-v1-readiness.md) | *(none — post-v1)* | Open. Four independent bug fixes, two tooling additions, and the `v0.2.0` surface's own `M5-2`-style ergonomics review (`M8-7`) — the review recommends, per [07 — Contributing](../07-contributing.md)'s issue-first rule, rather than deciding. Does not itself schedule the `v1.0.0` tag. |

All six checklist lines from [06 — Scope & Roadmap](../06-scope-and-roadmap.md) appear exactly once in the table above; `M5`, `M6`, `M7` and `M8` cover no checklist line, since v1's checklist is closed.
