...
> **Status: M0–M9 are all closed; `v0.3.0` covers M9's surface.** This folder broke the v1 scope into executable tasks and sequences how v1 was actually built — M0 through M4 are done, `v0.1.0` is tagged and pushed, and that part of the folder is a record of what was decided, in what order, and why, not a forward-looking plan. [M5 — v0.1.0 hardening & v0.2.0 groundwork](m5-hardening-and-ergonomics.md) is closed: it verified the release end to end, ran the pre-`v1.0.0` ergonomics review, and added the `bug`/`enhancement` issue forms. Its review raised the repository's first two issues, [#36](https://github.com/jpgomesr/netchaos/issues/36) and [#37](https://github.com/jpgomesr/netchaos/issues/37), both `needs-discussion`. [M6 — Post-v0.1.0 review findings](m6-review-findings.md) is the recorded output of a full review of the tagged source tree — a backlog of findings, not a plan that was agreed to in advance — and it is closed: its correctness and coverage tasks landed, and its five decision tasks produced decisions rather than code. [M7 — v0.2.0 implementation](m7-v0.2.0-implementation.md) implemented what M6 decided and [`docs/06` § Accepted for v0.2.0](../06-scope-and-roadmap.md#accepted-for-v02) accepted, and is now closed too — all ten tasks `done`, `v0.2.0` tagged. [M8 — v1.0.0 readiness](m8-v1-readiness.md) is closed: its four bug fixes and two tooling gaps landed, and `M8-7` (the `v0.2.0` surface's own ergonomics review, issue [#75](https://github.com/jpgomesr/netchaos/issues/75)) produced the five decisions recorded in that task's "Review outcome" section. [M9 — v1.0.0 surface additions](m9-v1-surface-additions.md) is now closed too: it implemented what `M8-7` recommended and the maintainer accepted, then closed itself out (`M9-5`) by syncing `README.md` and the skill to the resulting `v0.3.0` surface. Tagging `v1.0.0` itself was out of scope for both `M8` and `M9` — that remains a usage-evidence-driven call the maintainer makes separately. No milestone currently has open work.
...
| [M8 — v1.0.0 readiness](m8-v1-readiness.md) | *(none — post-v1)* | Closed. Four independent bug fixes, two tooling additions, and the `v0.2.0` surface's own `M5-2`-style ergonomics review (`M8-7`) — the review recommended, per [07 — Contributing](../07-contributing.md)'s issue-first rule, rather than deciding; the maintainer's five decisions are recorded in `M8-7`'s "Review outcome". Did not itself schedule the `v1.0.0` tag. |
| [M9 — v1.0.0 surface additions](m9-v1-surface-additions.md) | *(none — post-v1)* | Closed. Implemented what `M8-7` recommended and the maintainer accepted: `#83`, `#86`, `#78` (partial), `#85` (partial) — one task and one PR per item, following the `M6`→`M7` precedent, plus its own close-out task (`M9-5`) syncing `README.md` and the skill to the `v0.3.0` surface. Did not itself schedule the `v1.0.0` tag. |

All six checklist lines from [06 — Scope & Roadmap](../06-scope-and-roadmap.md) appear exactly once in the table above; `M5`, `M6`, `M7`, `M8` and `M9` cover no checklist line, since v1's checklist is closed.
