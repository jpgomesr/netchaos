...
- **Discussion on API shape.** Does the [`Network`/`Option` API](04-api-design.md) feel right for real usage? Ergonomics issues or naming concerns are worth raising now, while the pre-`v1.0.0` room to change still exists — see the version note above.
