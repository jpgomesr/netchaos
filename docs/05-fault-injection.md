...
**Static vs. dynamic:** `WithPartition(peerA, peerB)` establishes a partition at `Network` construction time, present for the whole test. `Network.Partition` / `Network.Heal` allow inducing and resolving a partition mid-test — useful for scenarios like "the connection was healthy, then the network split, then it recovered." Pairs are unordered: naming either peer first identifies the same partition. `Partition`/`Heal` (and `Network.Reset`, below) validate `peerA`/`peerB` the same way `WithPartition` does, panicking on an empty name or a self-pair ([M9-1](04-api-design.md#error-and-no-op-behaviour), issue [#83](https://github.com/jpgomesr/netchaos/issues/83)) — naming a peer that was never `Dial`ed or `Listen`ed still stays a silent no-op.
