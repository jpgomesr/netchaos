...
- **Half-close (`CloseRead`/`CloseWrite`)** — [issue #84](https://github.com/jpgomesr/netchaos/issues/84), decided by [M8-7 finding F3](tasks/m8-v1-readiness.md#f3--no-half-close-closereadclosewrite-issue-84). `conn.go` currently matches `net.Pipe`'s full-teardown `Close`, not a real socket's independent half-close, which is a real gap for code under test that type-asserts `interface{ CloseWrite() error }` (`io.Copy`-based proxies, several SSH/multiplexing libraries, HTTP/1.1 bodies). Adding it later is additive — a `*conn` simply starts satisfying a new interface — so nothing is lost by waiting for evidence that code under test actually needs it.
- **`Network.SetBandwidth`** — [issue #85](https://github.com/jpgomesr/netchaos/issues/85), decided by [M8-7 finding F6](tasks/m8-v1-readiness.md#f6--runtime-setters-exist-for-two-of-five-fault-kinds-issue-85). `SetDuplication`/`SetCorruption` were accepted (see "Accepted before v1.0.0" below); `SetBandwidth` was split out because, unlike every other fault kind, `WithBandwidth` draws nothing — it is a deterministic function of a unit's size and the configured rate — so a live setter's interaction with the pipe's serialization clock (`pipe.busyUntil`) needs its own design pass rather than an assumption that it mirrors `SetLatency`.
- **Disabling or clearing `Network.Trace`** — [issue #77](https://github.com/jpgomesr/netchaos/issues/77), decided by [M8-7 finding F7](tasks/m8-v1-readiness.md#f7--networktrace-has-no-disable-or-clear-issue-77). `Network.Trace` retains every fault event for the `Network`'s whole lifetime by design, with no way to opt out or prune — a real but narrow gap for a long-running or high-throughput test (a soak test, or a fuzz/property test dialing and writing many times per run). A `WithTrace(bool)` option and/or a `Network.ClearTrace()` method are both additive, so nothing here becomes harder or more breaking by waiting.
- **`FaultEvent` attribution of `Network.Reset`** — [issue #78](https://github.com/jpgomesr/netchaos/issues/78) (second half), decided by [M8-7 finding F4](tasks/m8-v1-readiness.md#f4--faultevent-cant-diagnose-a-corruptionduplication-failure-or-attribute-a-reset-issue-78). The payload-size and corruption-site fields F4 also covers were accepted (see "Accepted before v1.0.0" below); attributing a `Reset` call in the trace was split out because `Network.Reset` is an imperative action evaluated nowhere near the per-unit fault evaluator (`reset.go`), by design — recording it in the same trace is new plumbing, not a field add, and does not need to block anything.
...
## Accepted before v1.0.0

Decided by the maintainer against [M8-7](tasks/m8-v1-readiness.md#m8-7--api-ergonomics-review-of-the-v020-surface-the-v100-gate)'s recommendations — see that task's "Review outcome" section for the full reasoning per finding. Each item below was implemented under its own task and its own PR by [M9 — v1.0.0 surface additions](tasks/m9-v1-surface-additions.md), now closed and shipped as `v0.3.0`; this section stays the record of *what* was accepted and why, with a link to the task that implemented it.

- **Validate `Partition`/`Heal`/`Reset` the way `WithPartition` already does ([F2](tasks/m8-v1-readiness.md#f2--partitionhealreset-accept-a-self-pair-or-an-empty-peer-name-silently-unlike-withpartition-issue-83)).** All three runtime methods are to panic on an empty peer name or a self-pair, matching `validatePartitionPair`'s existing construction-time check — closing an asymmetry with no design rationale on record. Implemented by [M9-1](tasks/m9-v1-surface-additions.md#m9-1--83-validate-partitionhealreset-the-way-withpartition-already-does).
- **`DialerFor(name string, opts ...DialerOption)` with `WithDialTimeout` ([F5](tasks/m8-v1-readiness.md#f5--dialerfor-has-no-way-to-bound-a-wait-on-a-partitioned-peer-issue-86)).** Lets a dialer built through `DialerFor` — the drop-in path a client constructor with no context parameter has to use — fail fast against a partitioned peer instead of hanging until `Heal` or `go test`'s own timeout. Additive: `DialerFor(name)` without the option keeps today's unbounded-wait behaviour.
- **`FaultEvent` gains payload size and corruption site ([F4](tasks/m8-v1-readiness.md#f4--faultevent-cant-diagnose-a-corruptionduplication-failure-or-attribute-a-reset-issue-78), first half).** The byte/bit index `WithCorruption`'s `corruptionSite` already computes, plus the unit's payload size, both currently computed and discarded. A pure additive change to the exported `FaultEvent` struct.
- **`Network.SetDuplication` and `Network.SetCorruption` ([F6](tasks/m8-v1-readiness.md#f6--runtime-setters-exist-for-two-of-five-fault-kinds-issue-85), partial).** Matching `SetLatency`/`SetPacketLoss`'s live semantics and validation shape — both are Bernoulli draws with no serialization-clock interaction, unlike `SetBandwidth` (deferred above).
