...
Decided by the maintainer against the rubric below, from the candidates [M6](tasks/m6-review-findings.md) generated by applying that rubric to the shipped tree. **Accepted means in scope, not implemented** — each item below gets its own task and its own PR, and none of them has one yet.
